...
class StatusTransitionRuleTest {
    private val defaultStatusTransitionRule: StatusTransitionRule = StatusTransitionRule(::isTaskMovable)

    @Test
    fun `To Do 상태에서는 In Progress로 전이가 가능하다`() {
        // when
        val result = defaultStatusTransitionRule.isMovable(toDoTask, Status.IN_PROGRESS)
    ...
    fun `To Do 상태에서 담당자가 없다면 In Progress로 전이에 대해 담당자 없음을 반환한다`() {
        // when
        val result = defaultStatusTransitionRule.isMovable(toDoTaskWithoutAssignee, Status.IN_PROGRESS)
    ...
    fun `To Do 상태에서는 In Progress를 제외한 상태로 전이가 불가능하다`() {
        for (status in listOf(Status.TODO, Status.REVIEW, Status.DONE)) {
            // when
            val result = defaultStatusTransitionRule.isMovable(toDoTask, status)
    ...
    fun `In Progress 상태에서는 To Do와 Review로 전이가 가능하다`() {
        for (status in listOf(Status.TODO, Status.REVIEW)) {
            // when
            val result = defaultStatusTransitionRule.isMovable(inProgressTask, status)
    ...
    fun `Review 상태에서는 In Progress와 Done으로 전이가 가능하다`() {
        for (status in listOf(Status.IN_PROGRESS, Status.DONE)) {
            // when
            val result = defaultStatusTransitionRule.isMovable(reviewTask, status)
    ...
    fun `Done 상태에서 To Do로 전이가 가능하다`() {
        // when
        val result = defaultStatusTransitionRule.isMovable(doneTask, Status.TODO)
    ...
    fun `Done 상태에서 To Do를 제외한 상태로 전이가 불가능하다`() {
        for (status in listOf(Status.IN_PROGRESS, Status.REVIEW, Status.DONE)) {
            // when
            val result = defaultStatusTransitionRule.isMovable(doneTask, status)
    ...
    private val toDoTaskWithoutAssignee = Task(title = "우리땅!", tags = Tags(emptyList()), status = Status.TODO, user = User.None)
