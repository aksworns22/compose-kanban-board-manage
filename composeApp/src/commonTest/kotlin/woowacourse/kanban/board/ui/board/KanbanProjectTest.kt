...
class KanbanProjectTest {
    ...
    private val toDoTask: Task = Task(title = "우리땅!", tags = Tags(emptyList()), status = Status.TODO, user = users[2])
    ...
    private val doneTask: Task = Task(title = "우리땅!", tags = Tags(emptyList()), status = Status.DONE, user = users[3])
